...
void UKF::Initialization(Eigen::VectorXd& X, Eigen::MatrixXd& P, float time){  //初始化权重

	// std::cout<<"######################## UKFinitialize #######################"<<std::endl;
	x_ = X;   //用model_X,model_P初始化了modelP来自参数 ，跟X_hat,P_hat不一样
	P_ = P;
...
//
void UKF::Prediction(float ntime){ //根据模型对sigmapoints进行预测
	...
	for(int i=0; i<2*n_x_+1; ++i){ //遍历sigma点分别预测
		...
		if(n_x_ == 5)  //刚被初始化的时候是0
			yawd_k = sigma_points(4,i);
		if(n_x_ == 6){
			yawd_k = sigma_points(4,i);		//角速度
			a_k = sigma_points(5,i);  //加速度
		}
	...
	try{
		...
		for(int i=0; i<2*n_x_+1; ++i){
			...
			x_diff(3) =  normalangle(x_diff(3));//,x_diff(2)); 航向角归一化
...
void UKF::Update( std::vector<Eigen::VectorXd>& Z, const Eigen::VectorXd& beta, const float& last_beta){
	...
	T.fill(0.0);
	for(int i=0; i<2*n_x_+1; ++i){
		Eigen::VectorXd x_diff = sigma_points_pre.col(i)-x_;  //x_预测状态
		//x_diff(3) =  normalangle(x_diff(3));//,x_diff(2));
		Eigen::VectorXd z_diff = Z_sigma_.col(i)-z_pre_;  //Z_sigma_预测的量测sp-均值
	...
	for(int i = 0; i < Z.size() + 1; ++i)
	{
	    ...
	    if(i == Z.size()){
	    	...
	    	a = x_ + K * (Z.at(i) - z_pre_);  //状态更新
	    }

	    P_temp = P_temp + beta(i) * (a * a.transpose() - x_filter * x_filter.transpose()); //加权更新协方差
	}
...
void UKF::Update( Eigen::VectorXd& Z){
	...
	//std::cout<<"Update"<<std::endl;
	//最后更新
	Eigen::MatrixXd T = Eigen::MatrixXd(n_x_,n_z_);  //6*2
	T.fill(0.0);
	for(int i=0; i<2*n_x_+1; ++i){
		Eigen::VectorXd x_diff = sigma_points_pre.col(i)-x_;
		//x_diff(3) =  normalangle(x_diff(3));//,x_diff(2));
